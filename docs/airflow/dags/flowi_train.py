...
def create_spark_app_yaml(**kwargs):
    ...
    file_content = re.sub(r'\b[0-9a-f]{8}\b-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-\b[0-9a-f]{12}\b', flowi_run_uuid,
                          file_content)
...
running_preprocessing_task = spark_kubernetes.SparkKubernetesOperator(
    application_file='preprocessing_spark_application.yaml',
...
running_training_task = spark_kubernetes.SparkKubernetesOperator(
    application_file='training_spark_application.yaml',
    task_id='running_training',
    dag=dag
)
